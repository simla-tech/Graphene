...
extension Connection: Collection {
    ...
    public var startIndex: Int { self.edges.startIndex }
    public var endIndex: Int { self.edges.endIndex }
    ...
    public subscript(index: Int) -> T {
        ...
        set(newValue) {
            self.edges[index] = newValue
        }
    }

    // Method that returns the next index when iterating
    public func index(after i: Int) -> Int {
        self.edges.index(after: i)
    }
...
extension Connection.QueryKeys where T: Queryable {
    static func edges(_ builder: @escaping QueryBuilder<T>) -> Connection.QueryKeys {
        .init(Query(Connection.CodingKeys.edges) { (nodeBuilder: QueryContainer<Node<T>>) in
